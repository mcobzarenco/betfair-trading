...
from analytics import HorseModel, RiskModel2
import risk
...
class Jockey(Strategy):
    def __init__(self, db, vwao='vwao', train='train'):
        ...
        self.hm = HorseModel(beta=10.0, tau=2.0)
    ...
    def handle_race(self, race):
        ...
        if np.all(self.hm.get_runs(runners) > 0): #race['scheduled_off'] >= datetime.datetime(2012, 1, 3):
            vwao = self.vwao.ix[race['event_id']]['vwao'][runners].values
            q = 1.0 / vwao / np.sum(1.0 / vwao)
            p = self.hm.pwin(runners)
            ...
            [self.bet(race['event_id'], r, -w[i]) for i, r in enumerate(runners)]
...
if __name__ == '__main__':
    ...
    st = time.clock()
    algo.run('GB', datetime.datetime(2012, 1, 1), datetime.datetime(2013, 6, 1))
    en = time.clock()
    ...
    df.save('/home/marius/playground/betfair-trading/out/back4.pd')
